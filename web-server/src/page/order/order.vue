<template>
    <el-row style="height: 100%;">
      <!--  左侧订单详情   -->
        <el-col :span="7" class="left-wrapper" style="padding: 0;margin: 0;background-color: #fff;">
          <div style="overflow-y: hidden;background-color: #FFFFFF;" :style="{ height: carTableHeight + 'px' }" id="order-detail-wrapper">
            ...
            <div class="order-page-wrapper">
              ...
              <div style="background-color: #FFFFFF;padding-left: 18px;padding-right: 18px;">
                ...
                <ul class="car-title">
                  <li class="car-title-item" style="width: 20%; text-align: left; text-indent: 0px;">选项</li>
                  <li class="car-title-item" style="width: 30%;">品名</li>
                  <li class="car-title-item" style="width: 15%;">数量</li>
                  <li class="car-title-item" style="width: 30%;text-align:right;">小计</li>
                </ul>
                ...
                <ul class="car-title car-content" v-for="(article, index) in car"  @click="checkOne(article, index)">
                  ...
                  <div v-show="(article.type == 3 && !!article.tempRefundCount)? false: true ">
                    ...
                    <li class="car-title-item car-content-item" style="width: 22%;vertical-align: top;text-align: right;">¥{{formatMoney(article.price)}}</li>
                  </div>
                  ...
                  <div v-if="article.type==3 && article.tempRefundCount" style="margin-left: 5px;" v-for="(item, index) in article.mealItems" :key="index">
                    <li class="car-title-item car-content-item" style="width: 15%;text-align: center;visibility: hidden">
                    <li class="car-title-item car-content-item" style="width: 40%;">
          ...
          <div class="car-footer">
            <el-row  type="flex" justify="space-between" class="order-operate-button" style="margin-left: 18px;margin-right: 18px;">
              <el-col :span="5" >
            ...
            <el-row style="text-align: center;" type="flex" justify="space-around" class="order-operate-button">
              <el-col :span="12" style="text-align: center;">
                <el-button style="width: 70%;" class="operate-button" @click="cancel" v-if="orderInfo.distribution_mode_id == 3">取消</el-button>
                <el-button style="width: 70%;" class="operate-button" @click="goBack" v-else>返回</el-button>
              </el-col>
              <el-col :span="12" style="text-align: center;">
                <el-button style="width: 70%;" class="operate-button operate-button-push-order" @click="checkedOrder"  :disabled="countDown>0"  v-if="shopDet.allow_first_pay == 0 || orderInfo.distribution_mode_id == 3">结算{{countDown}}</el-button>
                <el-button style="width: 70%;" class="operate-button operate-button-push-order" @click="checkedOrder"  :disabled="countDown>0"  v-else>下单</el-button>
              </el-col>
            </el-row>
          </div>
        </el-col>
          ...
          <el-col :span="20" class="article-info" id="articleInfo">
            <el-row :gutter="5" style="height: 100%">
              <div class="article-page-wrapper">
                <div class="pre-page" @click="articlePageOperation(-1)"><span class="el-icon-caret-top" style="font-size: 26px;margin-left: 2px;"></span></div>
      ...
      <el-dialog :visible.sync="articleWeightPackageDialog.show" size="large" :close-on-click-modal="false" class="el-dialog__wrap">
        ...
        <div class="dialog-content scrollbar" >
          <ul v-for="weightPackagekList in articleWeightPackageDialog.article.weightPackagekList" :key="weightPackagekList.id">
            <li>
        ...
        <span slot="footer">
          ...
          <el-button @click="articleWeightPackageDialog.show = false">取 消</el-button>
          <el-button type="primary" @click="addArticleWeightPackageToCar(articleWeightPackageDialog.article)" :disabled="addCar">确 定</el-button>
        </span>
      </el-dialog>
      <!--  称重包   end  -->
    </el-row>
  ...
  <script>
    ...
    import keyboard from '../../page/stock/components/keyboard'
    import smallkeyboard from '../../components/keyboard/smallkeyboard'
    import bus from '../../utils/bus'
    import index from '../../properties'
    import CarDetail from '../../components/basic/car-detail.vue'
    ...
    export default {
      name: 'order',
      mixins:[mixin],
      components: {keyboard, smallkeyboard, CarDetail},
      data() {
        return {
          isEditArticle: true,
          editArticleInfo: {},
      ...
      methods: {
        ...
        initOrder() {
          let that = this;
          getOrderDetail(this.$route.params.orderId, function (data) {
            that.orderInfo = data;
            // this.$store.dispatch('getUnitListByArticleId', currentArticle);
            // that.$store.commit('init_main', that.orderInfo)
        ...
        pushOrder(toPayOrder) {  // 下单
          ...
          this.tableOrder.afterPay = this.orderInfo.pay_type;
          ...
          for (let item of this.car) {
            if (item.state === 0) {
              ...
              item.mealFeeNumber = this.orderInfo.distribution_mode_id == 3 ? ( is_meal_fee ? item.mealFeeNumber * item.count : 0) : 0
              if(item.count == 0) {
          ...
          this.mealAllNumber = this.orderInfo.distribution_mode_id == 3 ? ( is_meal_fee ? this.mealAllNumber : 0) : 0;
          let pushOrderInfo = {
  ...
  //        let mealFeePriceOne = 1;
          ...
          let addArticle = that.$route.query.addArticle
          if(addArticle) { // 如果是继续点餐
            let suborderId = this.$route.query.suborderId || null;
            if (suborderId) pushOrderInfo.masterOrderId =  suborderId
            addOrderItemToOrder(pushOrderInfo, function (res) {
              that.$router.push("/pay/" + that.orderInfo.id + (suborderId ? "?suborderId=" + suborderId : ""));
            })
          } else if (this.orderInfo.order_item_list.length>0) { // 如果订单信息中有order_item_list，这个订单是第二次下单，如果是外带模式，则不能有加菜
            getOrderDetail(this.orderInfo.id, (currentOrderInfo) => {
              if(currentOrderInfo.order_state == 2 && this.shopModel == 6 && this.shopDet.allow_after_pay == 0 ) {
                return this.$notify.warning("订单状态发生变更")
              }

              if (orderItem.length === 0) return  this.$message.error('请添加菜品')
              if (this.orderInfo.distribution_mode_id == 3) return  this.$message("对不起，外带不能加菜");
              this.tableOrder.childrenOrder = 1;
              if(this.orderInfo.data_origin == 0) {
                bindTable(this.tableOrder,'', function (childrenOrderId) {
                  pushOrderInfo.childrenOrderId = childrenOrderId
                  pushOrder(pushOrderInfo, function () {
                    that.pushOrderSuccess(that.orderInfo, childrenOrderId);
                  });
                });
              } else {// 本步逻辑是 加菜情况下，先验证微信端是否正在支付，如果是，再验证微信端是否正在加菜
                if(navigator.onLine) {
                  let verInfo = {
                  ...
                  verificationOfOrders(verInfo, function (result) {
                    ...
                    if (data.success) {
                      ...
                      verificationOfOrders(verInfo, (result) =>{
                        ...
                        if(data.success) {
                          bindTable(that.tableOrder,'', function (childrenOrderId) {
                            pushOrderInfo.childrenOrderId = childrenOrderId
                            pushOrder(pushOrderInfo, function () {
                              // that.syncOrderInfoToServer(childrenOrderId);
                              that.pushOrderSuccess(that.orderInfo, childrenOrderId);
                            });
                          });
                        } else {
            ...
            pushOrder(pushOrderInfo, function (data) {//父id，子Id,订单菜品项
              // that.syncOrderInfoToServer(that.orderInfo.id);
              that.pushOrderSuccess(that.orderInfo);
            });
          }
        },
        // syncOrderInfoToServer(orderId){
        //   let that = this;
        //   getOrderDetail(orderId, function (data) {
        //     that.sendOrderInfo(data);
        //   });
        // },
        pushOrderSuccess(order, addOrderId){
          // 先付 || 外带  ==>  直接跳转支付页面
          if(this.shopDet.allow_first_pay == 0 || order.distribution_mode_id == 3){
            this.$router.push("/pay/" + order.id + (addOrderId ? "?suborderId=" + addOrderId : ""));
          }else if(this.shopDet.allow_after_pay == 0){ // 后付
            ...
            // this.$notify.success({ message: '下单成功！', duration: 1500 });
          }
