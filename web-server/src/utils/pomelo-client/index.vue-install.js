...
pomeloClient.install = function (Vue, options) {
  // 避免重复 安装
  if (pomeloClient.install.installed) {
    return
  }
  pomeloClient.install.installed = true
  ...
  // 初始化 pomelo
  var POMELO_HOST = options.host || '127.0.0.1'
  var POMELO_PORT = options.port || 3050
  pomelo.init({host: POMELO_HOST, port: POMELO_PORT}, function () {
    console.log('pomelo client init success ...')
    // 导出
